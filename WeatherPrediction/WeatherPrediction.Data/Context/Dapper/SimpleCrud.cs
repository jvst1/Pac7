...
namespace WeatherPrediction.Data.Context.Dapper
{
    public static class SimpleCrud
    {
        ...
        private static readonly bool _stringBuilderCacheEnabled = true;
        ...
        {
            if (_stringBuilderCacheEnabled && StringBuilderCacheDict.TryGetValue(cacheKey, out string value))
            {
                sb.Append(value);
                return;
            }
        ...
        private static bool ValidateKeyType(Type keytype)
        {
        ...
        public static void Insert<TEntity>(this IDbConnection connection, TEntity entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var idProps = GetIdProperties<TEntity>().ToList();

            if (!idProps.Any())
                throw new ArgumentException("Insert<T> only supports an entity with a [Key] or Id property");

            var baseType = idProps.First().PropertyType;
            ...
            connection.Query(sb.ToString(), entityToInsert, transaction, true, commandTimeout);
        }
        ...
        /// <param name="commandTimeout"></param>
        /// <returns>The ID (primary key) of the newly inserted record if it is identity using the defined type, otherwise null</returns>
        public static void BulkInsert<TEntity>(this IDbConnection connection, IEnumerable<TEntity> entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var idProps = GetIdProperties<TEntity>().ToList();
            ...
            var properties = GetAllPropertiesBulkInsert<TEntity>();

            var tableName = GetTableName(entityToInsert.First());

            using var bulkCopy = new SqlBulkCopy(connection.ConnectionString);
            var dataTable = ToDataTable(entityToInsert, properties);
        ...
        public static void BulkInsert<TEntity>(this SqlConnection connection, IEnumerable<TEntity> entityToInsert, SqlTransaction transaction = null, int? commandTimeout = null)
        {
            var idProps = GetIdProperties<TEntity>().ToList();
        ...
        public static void BulkUpdate<TEntity>(this IDbConnection connection, IEnumerable<TEntity> entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var idProps = GetIdProperties<TEntity>().ToList();
            ...
            var properties = GetAllPropertiesWithValueBulkUpdate(entityToInsert.First());
            var tableName = GetTableName(entityToInsert.First());
            var tempTableName = $"#SafeTeste";

            using var conn = new SqlConnection(connection.ConnectionString);

            CreateTempTable(conn, tempTableName, properties);

            ExecuteBulkCopy(conn, entityToInsert, properties, tempTableName);

            CommandUpdate(tableName, tempTableName, properties, conn, idProps);

            conn.Close();
        }

        public static void BulkUpdate<TEntity>(this SqlConnection connection, IEnumerable<TEntity> entityToInsert, SqlTransaction transaction = null, int? commandTimeout = null)
        {
            var idProps = GetIdProperties<TEntity>().ToList();
            ...
            CreateTempTable(connection, tempTableName, properties, transaction);

            ExecuteBulkCopy(connection, entityToInsert, properties, tempTableName, transaction);

            CommandUpdate(tableName, tempTableName, properties, connection, idProps, transaction);
        }

        private static void CommandUpdate(string tableName,
        ...
        {
            var listaColunas = new List<string>();
            foreach (var prop in properties.Where(x => !idProps.Contains(x)))
                listaColunas.Add($"{prop.Name} = Temp.{prop.Name}");

            var sb = new StringBuilder();
            sb.Append($"update {tableName} set ");
            sb.Append(string.Join(",", listaColunas));
            sb.Append($" FROM {tableName} T ");
            sb.Append($" inner join {tempTableName} Temp on T.{idProps.First().Name} = Temp.{idProps.First().Name};");
            ...
            commandUpdate.CommandText = sb.ToString();
            commandUpdate.ExecuteNonQuery();
        }

        private static void ExecuteBulkCopy<TEntity>(SqlConnection connection,
            IEnumerable<TEntity> entityToInsert,
            List<PropertyInfo> properties,
            string tableName,
            SqlTransaction transaction = null)
        {
            ...
            SqlTransaction transaction = null)
        {
            ...
            foreach (var prop in properties)
                listaColunas.Add($"{prop.Name} {ConvertCSharpFormatToSqlServer(prop.PropertyType.Name.ToString())}");

            var sql = $"create table {tempTableName} ( {string.Join(",", listaColunas)} )";
        ...
        private static DataTable ToDataTable<T>(IEnumerable<T> data, IList<PropertyInfo> properties)
        {
            ...
            {
                typeCasts[i] = properties[i].PropertyType.IsEnum ? Enum.GetUnderlyingType(properties[i].PropertyType) : null;
            }
        ...
        private static void BuildWhere<TEntity>(StringBuilder sb, IEnumerable<PropertyInfo> idProps, object whereConditions = null)
        {
            ...
            for (var i = 0; i < propertyInfos.Count(); i++)
            {
                ...
                for (var x = 0; x < sourceProperties.Count(); x++)
                {
                    ...
                    {
                        if (whereConditions != null && propertyToUse.CanRead && (propertyToUse.GetValue(whereConditions, null) == null ||
                ...
                sb.AppendFormat(useIsNull ? "{0} is null" : "{0} = @{1}", GetColumnName(propertyToUse), propertyToUse.Name);
        ...
        private static List<PropertyInfo> GetAllPropertiesWithValueBulkUpdate<T>(T obj)
        {
            ...
            for (var i = 0; i < props.Count(); i++)
            {
                ...
                if (property.GetCustomAttributes(true).Any(attr =>
                    attr.GetType().Name == typeof(IgnoreInsertAttribute).Name ||
                    attr.GetType().Name == typeof(NotMappedAttribute).Name ||
                    attr.GetType().Name == typeof(IgnoreBulkUpdate).Name ||
                    attr.GetType().Name == typeof(ReadOnlyAttribute).Name && IsReadOnly(property))) continue;
                ...
                if (value != default)
                {
                    ...
                    if (property.PropertyType == typeof(decimal) && (decimal)value == decimal.Zero)
                        continue;
        ...
        private static List<PropertyInfo> GetAllPropertiesBulkInsert<T>()
        {
            var lista = new List<PropertyInfo>();
            var props = GetScaffoldableProperties<T>().ToArray();
        ...
        private static IEnumerable<PropertyInfo> GetIdProperties<T>()
        {
            var type = typeof(T);
            var mapping = DbProvider.FindDbMapping(type);
            var idMemberName = mapping.GetCodigo().GetMemberName();

            var tp = type.GetProperties().Where(p => p.Name == idMemberName).ToList();
            return tp.Any() ? tp : type.GetProperties().Where(p => p.Name.Equals("Codigo", StringComparison.OrdinalIgnoreCase));
        }
            ...
            public virtual string ResolveTableName(Type type)
            {
                ...
                if (mapping != null)
                {
                    try
                    {
                        ...
                        {
                            var schemaName = Encapsulate(mapping.Schema);
